export const useAspirasi = () => {
  ...
  const hapusAspirasi = async (
    ...
    try {
      const res = await $fetch<ApiResponse<null>>(`/api/aspirasi/${id}`, {
        method: "DELETE",
        headers,
      });

      aspirasi.value = aspirasi.value.filter((item) => item.id !== id);
      return res;
    } catch (err: any) {
      error.value = err.data?.message ?? "Gagal menghapus aspirasi";
      throw err;
    } finally {
      loading.value = false;
    }
